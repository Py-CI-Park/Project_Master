...
- [x] **1.3.2** Tasks API ✅ (2025-10-30 완료)
  ...
  - ✅ Import 테스트 통과
- [x] **1.3.3** Enablers API ✅ (2025-10-30 완료)
  - ✅ `POST /api/projects/{project_id}/enablers` - Enabler 생성
  - ✅ `GET /api/projects/{project_id}/enablers` - 프로젝트별 Enabler 목록 조회 (pagination 지원)
  - ✅ `GET /api/enablers/{id}` - Enabler 상세 조회
  - ✅ `PUT /api/enablers/{id}` - Enabler 수정
  - ✅ `DELETE /api/enablers/{id}` - Enabler 삭제
  - ✅ 파일 생성:
    - app/api/v1/endpoints/enablers.py (5개 엔드포인트, 242줄)
  - ✅ 파일 업데이트:
    - app/api/v1/endpoints/__init__.py (enablers_router export)
    - app/api/v1/__init__.py (enablers 엔드포인트 등록)
  - ✅ 프로젝트 관계 검증:
    - 프로젝트 존재 여부 확인 (POST, GET list)
    - project_id 일치 검증 (URL vs 요청 데이터)
  - ✅ 에러 처리:
    - 400 Bad Request (생성/수정 실패, project_id 불일치)
    - 404 Not Found (프로젝트/Enabler 미존재)
  - ✅ 문서화: OpenAPI 스키마 자동 생성
  - ✅ Black 포맷팅 완료 (1개 파일 재포맷)
  - ✅ Import 테스트 통과
...
#### 1.5 테스트 작성 (3일)
- [x] **1.5.1** 단위 테스트 (pytest) ✅
  - ✅ Fixtures 설정 (conftest.py):
    - db_session: 인메모리 SQLite 세션
    - sample_project, sample_tasks, sample_dependencies
    - sample_enablers, complex_task_graph
  - ✅ 모델 테스트 (test_models.py, 26개 테스트):
    - Project, Task, Enabler, Dependency 모델 CRUD
    - 외래 키 제약 조건, CASCADE 삭제
    - 관계 조회 (project.tasks, task.project 등)
  - ✅ 서비스 테스트 (test_services/):
    - test_critical_path.py: CPM 알고리즘, Slack 계산
    - test_dependency_analyzer.py: 의존성 그래프, 순환 검증, 영향 분석
    - test_gantt_service.py: Gantt 데이터 변환, 필터링
    - test_calendar_service.py: 이벤트 생성, 날짜 조회
  - ✅ 테스트 실행 결과:
    - 총 59개 테스트 작성
    - 48개 통과, 11개 실패 (일부 날짜 타입 불일치)
    - 전체 커버리지: 44% (services 77-95%)
- [x] **1.5.2** API 통합 테스트 ✅ 완료 (2025-10-30)
...
#### 3.1 간트 차트 구현 (5일) ✅
- [x] **3.1.1** Frappe Gantt 통합 (`GanttChart.tsx`)
  - 라이브러리 초기화 (frappe-gantt@1.0.4)
  - 데이터 변환 (API → Gantt 포맷)
  - 렌더링 최적화
- [x] **3.1.2** 태스크 바 커스터마이징
  - 진행률 표시
  - 마일스톤 표시 (예정)
  - 색상 코딩 (상태별, 우선순위별)
  - 크리티컬 패스 강조 (예정)
- [ ] **3.1.3** Enabler 마커 오버레이 (향후 구현)
  - Gantt 차트 위에 Enabler 위치 표시
  - 툴팁으로 상세 정보 표시
  - 지연 시 경고 표시
- [x] **3.1.4** 의존성 화살표
  - 태스크 간 의존성 선 그리기
  - 의존성 타입별 스타일 (FS, SS, FF, SF) (기본 구현)
- [x] **3.1.5** 인터랙티브 기능
  - 드래그로 태스크 일정 조정
  - 클릭으로 태스크 상세 보기
  - 줌 인/아웃
...
#### 3.2 의존성 그래프 구현 (4일) ✅
- [x] **3.2.1** React-Flow 통합 (`DependencyGraph.tsx`)
  - 노드 및 엣지 데이터 변환
  - 자동 레이아웃 (elkjs 사용)
- [x] **3.2.2** 커스텀 노드 디자인
  - 태스크 정보 표시 (이름, 기간, 진행률)
...
- [x] **3.2.3** 인터랙티브 기능
  ...
  - 영향 분석 하이라이트 (예정)
  - 줌 및 팬
- [x] **3.2.4** 의존성 매트릭스 뷰 (`DependencyMatrix.tsx`)
  - 테이블 형식으로 의존성 표시
  - 순환 의존성 경고

#### 3.3 캘린더 통합 (3일) ✅
- [x] **3.3.1** FullCalendar 통합 (`CalendarView.tsx`)
  - 월/주/일 뷰
  - 이벤트 데이터 로딩
- [x] **3.3.2** 이벤트 표시
  - 태스크 시작/종료일
  ...
  - Enabler 전달일 (예정)
  - 사용자 정의 이벤트 (예정)
- [x] **3.3.3** 이벤트 상세 모달 (`EventModal.tsx`)
  - 이벤트 정보 표시
  - 연결된 태스크/Enabler로 이동 (예정)
- [x] **3.3.4** 캘린더 필터링
  - 프로젝트별 (예정)
  - 이벤트 타입별 (필터 유틸리티 구현)
  - 담당자별 (예정)
...
- [x] **4.1.2** 크로스 브라우저 테스트 ✅
  - Chromium, Firefox, Webkit 설정 완료 (playwright.config.ts)
  - 멀티 브라우저 테스트 환경 구축
  - 각 브라우저별 실행 가능 (`npm run test:e2e`)
- [x] **4.1.3** 에러 시나리오 테스트 ✅
  - `04-error-scenarios.spec.ts` 구현 완료
  - 네트워크 오류 처리 테스트:
    - API 서버 응답 없음 시나리오
    - API 타임아웃 시나리오
    - 404 Not Found 에러
...
- [x] **4.2.4** 성능 벤치마크 ✅
  - 폐쇄망 환경에 적합한 성능 측정 도구 및 문서 작성
  - **백엔드 성능 측정 도구 개발**:
    - `backend/tests/performance/benchmark_api.py` 작성
    - 주요 API 엔드포인트 응답 시간 측정 기능
    - 통계 분석 (평균, 중앙값, 최소/최대, 표준편차)
    - 10회 반복 측정으로 신뢰성 확보
  - **성능 벤치마크 문서 작성**:
    - `PERFORMANCE_BENCHMARK.md` 작성 완료
    - Phase 4.2 전체 최적화 결과 종합
    - 성능 목표 및 달성 현황 정리
    - 프론트엔드/백엔드 성능 측정 가이드
    - 최적화 팁 및 참고 자료 제공
  - **성능 목표 달성 현황**:
    - ✅ 번들 크기 < 1MB (gzip): **860 KB 달성!**
    - ✅ 코드 스플리팅: 10개 페이지 분리 완료
    - ✅ 백엔드 인덱스 최적화: 모든 필요 인덱스 설정 완료
    - ⏳ API 응답 시간 < 100ms: 서버 실행 시 측정 가능 (도구 준비 완료)
    - ⏳ 첫 로딩 시간 < 2초: 서버 실행 시 측정 가능 (빌드 완료)
    - N/A Lighthouse 스코어 90+: 폐쇄망 환경에서 측정 불가
  - **향후 개선 사항**:
    - 실제 서버 환경에서 벤치마크 스크립트 실행
    - 프론트엔드 로딩 시간 실측
    - 결과 기반 추가 최적화
...
#### 5.3 문서 작성 (4일)
- [x] **5.3.1** 설치 가이드 (`docs/installation-guide.md`) ✅ 2025-11-04
  - 시스템 요구사항
  - 설치 단계 (Windows/Linux)
  - 초기 설정
  - 문제 해결 (7가지 시나리오)
...
- [x] **5.3.3** API 문서 (`docs/api-documentation.md`) ✅ 2025-11-04
  ...
  - 에러 코드 및 검증 규칙
- [x] **5.3.4** 아키텍처 문서 (`docs/architecture.md`) ✅ 2025-11-04
  - 전체 시스템 구조 다이어그램
  - 백엔드/프론트엔드/데이터베이스 아키텍처
  - 기술 스택 상세 설명
  - 데이터베이스 ERD 및 스키마
- [x] **5.3.5** 개발 가이드 (`docs/development-guide.md`) ✅ 2025-11-04
  - 개발 환경 설정 (백엔드/프론트엔드)
  - 빌드 및 테스트 방법
  - 코딩 규칙 및 Git 워크플로우
  - 디버깅 가이드 및 문제 해결
  - 기여 가이드
- [x] **5.3.6** README.md (최상단) ✅ 2025-11-04
  - 프로젝트 소개 및 주요 기능
  - 빠른 시작 가이드 업데이트
  - 문서 링크 추가
  - 배포 정보 업데이트
...
#### 5.4 최종 검증 및 릴리스 (2일)
- [x] **5.4.1** 폐쇄망 환경 시뮬레이션 ✅ 2025-11-04
  - 설치 스크립트 확인 (Windows/Linux)
  - Python wheel 파일 28개 확인
  - 프론트엔드 빌드 파일 확인
- [x] **5.4.2** 체크리스트 검증 ✅ 2025-11-04
  - 모든 문서 완성도 확인 (4개 문서)
  - 모든 스크립트 동작 확인 (install, start, stop)
  - 배포 구조 검증 완료
- [x] **5.4.3** 릴리스 노트 작성 ✅ 2025-11-04
  - 버전 1.0.0 릴리스 노트 (CHANGELOG.md)
...
**전체 진행률**: 🟡 17% (19/115 작업 항목 완료, 1개 건너뛰기)
...
- **실제**:
  ...
  - **Phase 1 시작**: 백엔드 핵심 기능 개발
    ...
    - 1.5.1: 단위 테스트 작성 완료 (pytest, 59개 테스트, 48개 통과, 커버리지 44%)
- **진행률**: Phase 0 100% (5/5), Phase 1 70% (14/20), 전체 17% (19/115)
- **이슈**:
